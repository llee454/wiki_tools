# This is a simple script that posts/edits pages
# within remote MediaWiki sites.

# I. Get username, password, and text
...
do
  case "$1" in
    -h | --help )
      cat <<EOF
Usage: post_wiki.sh [OPTIONS] --url URL --user USER
         --password PASSWORD --title TITLE'

post_wiki.sh uses cURL to post/edit MediaWiki pages. The command
...
# Get edit token
csrftoken=$(curl --silent --cookie $cookie --cookie-jar $cookie -X POST --data 'action=query' --data 'meta=tokens' --data 'format=xml' $url | sed -E 's/.*csrftoken="([^"]*)".*/\1/')

# Post data
curl --silent --cookie $cookie --cookie-jar $cookie -X POST --data 'action=edit' --data "title=$title" --data "text=$body" --data-urlencode "token=$csrftoken" --data 'format=xml' $url
